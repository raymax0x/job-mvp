  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4">
        {currentJobs.map((job) => (
          <div key={job.id} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
            <div className="p-5 cursor-pointer hover:bg-gray-50" onClick={() => toggleExpand(job.id)}>
              <div className="flex flex-wrap md:flex-nowrap justify-between">
                ...
                <div className="w-full md:w-3/5 pr-4">
                  ...
                  <div className="text-sm text-gray-600 mb-3">
                    {job.company?.display_name || 'Unknown Company'} • {job.location?.display_name || 'Unknown Location'}
                  </div>
                ...
                <div className="w-full md:w-2/5 flex flex-col items-end justify-between mt-4 md:mt-0">
                  <div className="grid grid-cols-2 gap-3 text-center w-full md:w-auto">
                    <div className="flex flex-col items-center">
                      <span className="font-bold text-xl text-blue-600">76</span>
                      <span className="text-xs text-gray-500">Match Score</span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="font-bold text-xl text-green-600">24</span>
                      <span className="text-xs text-gray-500">New Skills</span>
                    </div>
                  </div>
                  <div className="mt-4">
                    <a 
                  ...
                  <div>
                    {coverLetters[job.id] ? (
                      <div className="bg-white p-4 border border-green-200 rounded-md">
                        <h4 className="font-semibold mb-2 text-gray-800">Cover Letter</h4>
                        <p className="text-gray-700 whitespace-pre-wrap">{coverLetters[job.id]}</p>
                      </div>
                    ) : (
                      <button
                        onClick={(e) => {
